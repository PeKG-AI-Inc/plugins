#!/usr/bin/env bash
# PeKG PermissionRequest hook for Claude Code.
# Abilities: A8 auto-deny on permission asks when active blockers exist + tool is mutating.
# Distinct from PreToolUse: this fires when Claude Code's permission system asks
# for user/policy approval. We answer before the user sees a dialog.
...
allow_passthrough() {
  # Return nothing — Claude Code falls back to its normal permission flow.
  exit 0
}
